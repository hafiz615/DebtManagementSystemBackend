...
class PipelineStatusController {
  ...
  getPipelineDetails = async (req: Request, res: Response) => {
    try {
      const checkPermission = await commonUtil.checkPermission(
        'viewPipeline',
        req
      ...
      return res.status(constants.CODE.OK).send(
        responseHelper.get2xxResponse({
          statusCode: constants.CODE.OK,
          data: response[1],
          message: constants.successFoundMessage('Pipeline details'),
        })
      );
    } catch (error: any) {
      console.log(error);
  ...
  getCasesByCustomFieldAndValue = async (req: Request, res: Response) => {
    try {
      const response =
        await this.pipelineStatusService.getCasesByCustomFieldAndValue(req);
      if (!response[0]) {
        return res
          .status(constants.CODE.OK)
          .send(responseHelper.get4xxResponse(response[1]));
      ...
      return res.status(constants.CODE.OK).send(
        responseHelper.get2xxResponse({
          ...
          message: constants.successFoundMessage(
            'Cases with custom field and value'
          ),
        })
      );
    } catch (error: any) {
