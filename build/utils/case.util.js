...
const debtor_service_1 = __importDefault(
  require('../api/services/debtor.service')
);
const creditor_service_1 = __importDefault(
  require('../api/services/creditor.service')
);
...
const axiosInstanceInterceptor_1 = __importDefault(
  require('./axiosInstanceInterceptor')
);
...
class CaseUtil {
  constructor() {
    ...
    this.paymentLoggingRepository =
      new paymentLogging_repository_1.PaymentLoggingRepository();
  ...
  async createContacts(data) {
    ...
    for (const contact of data) {
      ...
      const validatedContact = dataCopier_util_1.DataCopier.copy(
        newContact,
        contact
      );
  ...
  async createCreditor(data) {
    ...
    const validatedCreditor = dataCopier_util_1.DataCopier.copy(
      newCreditor,
      data
    );
  ...
  async checkCasePayment(body) {
    if (body.remaining && body.remaining !== body.totalDebt - body.paidAmount) {
      return [
        false,
        constants_util_1.default.Messages.PAYMENT_CALCULATION_ERROR,
      ];
    }
  ...
  async getClientDetails(req) {
    const convertedDebtorId = new mongoose_1.default.Types.ObjectId(
      req.params.id
    );
    const pipeline = [
      {
  ...
  async getCreditorDetails(req) {
    const convertedCreditorId = new mongoose_1.default.Types.ObjectId(
      req.params.id
    );
    const pipeline = [
      {
  ...
  async getCreditorNamesAI(
    ...
    try {
      ...
      const response = await axiosInstanceInterceptor_1.default.post(
        url,
        data,
        {
          headers: {
  ...
  async getSummary(req, caseTemp) {
    if (
      ...
      new Date(global_1.AIAuth.expires_in) <=
  ...
  async createCreditorsCases(req, name, id) {
    ...
    for (const body of dataArray) {
      ...
      if (getCreditor) {
        creditor = await this.creditorRepository.updateById(
          getCreditor._id,
          body.creditor
        );
      }
