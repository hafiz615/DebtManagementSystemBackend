...
class PaymentController {
    constructor() {
        this.getHomePayments = async (req, res) => {
            try {
                const response = await this.paymentService.getHomePayments(Number(req.query.days));
                if (!response[0]) {
                    return res
        ...
        this.getCaseUpcomingPayments = async (req, res) => {
            try {
                const response = await this.paymentService.getCaseUpcomingPayments(req.params.id);
                if (!response[0]) {
                    return res
                        .status(constants_util_1.default.CODE.BAD_REQUEST)
                        .send(responseHelper_util_1.default.get4xxResponse(response[1]));
                ...
                return res.status(constants_util_1.default.CODE.OK).send(responseHelper_util_1.default.get2xxResponse({
                    statusCode: constants_util_1.default.CODE.OK,
                    data: response[1],
                    message: constants_util_1.default.successFoundMessage('Upcoming payments'),
                }));
            }
            catch (error) {
                console.log(error.message);
